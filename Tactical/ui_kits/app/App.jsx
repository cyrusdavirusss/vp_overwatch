...
function App() {
  ...
  // Onboarding gate — show on first launch (localStorage); dismiss to map.
  const [showOnboarding, setShowOnboarding] = useStateA(() => !hasSeenOnboarding());

  const [theme, setTheme] = useStateA(t.theme || 'dark');
  useEffectA(() => { setTheme(t.theme); }, [t.theme]);
  ...
  return (
    <div className="vp-app" style={{ width: MAP_W, height: SCREEN_H, position: 'relative', overflow: 'hidden', background: 'var(--ink-0)' }}>
      ...
      {/* Tweaks panel */}
      <TweaksPanel title="VP-Overwatch · Tweaks">
        ...
        <TweakSection title="Map style">
          <TweakSelect
            value={t.mapStyle}
            options={[['Navigation Night','night'],['Toner Mono','toner'],['Hypsometric','terrain']]}
            onChange={(v) => setTweak('mapStyle', v)}
          />
        </TweakSection>
        <TweakSection title="Scrubber density">
          <TweakRadio
            value={t.density}
        ...
        <TweakSection title="Aircraft visual">
          <TweakToggle label="Predictive vector cone" checked={t.showPredictive} onChange={(v) => { setTweak('showPredictive', v); setFilters(f => ({ ...f, predictive: v })); }} />
          <TweakToggle label="Aircraft trails" checked={t.showTrails} onChange={(v) => { setTweak('showTrails', v); setFilters(f => ({ ...f, trails: v })); }} />
          <TweakToggle label="Heatmap overlay" checked={t.showHeatmap} onChange={(v) => { setTweak('showHeatmap', v); setFilters(f => ({ ...f, heatmap: v })); }} />
        </TweakSection>
        <TweakSection title="First run">
          <TweakButton onClick={() => { clearOnboarded(); setShowOnboarding(true); }}>Re-show onboarding</TweakButton>
        </TweakSection>
        <TweakSection title="Demonstrable states">
          <TweakButton onClick={() => { setScrubT(0); setSelectedAircraftId(null); setSelectedReportId(null); setFilterOpen(false); setInspectPos(null); }}>Live now</TweakButton>
          <TweakButton onClick={() => { setScrubT(1200); setSelectedAircraftId(null); setSelectedReportId(null); }}>Scrub back 20m</TweakButton>
          <TweakButton onClick={() => { setScrubT(0); setSelectedAircraftId(data.AIRCRAFT[0].id); }}>Aircraft selected</TweakButton>
          <TweakButton onClick={() => { setScrubT(0); setSelectedReportId(data.REPORTS[1].id); }}>Report selected</TweakButton>
          <TweakButton onClick={() => { setFilterOpen(true); }}>Filters open</TweakButton>
          <TweakButton onClick={() => { setInspectPos({ x: 800, y: 200, sx: 220, sy: 380 }); }}>Inspect pin</TweakButton>
        </TweakSection>
      </TweaksPanel>
